...
public static class NumericCollectionAssertionsExtensions
{
        ...
        where T : INumber<T>
    {
        ...
        assertionChain
            ...
            .WithExpectation("Expected {context:collection} to approximate {0} by ± {1}{reason}, ", expectedItems, precision, chain => chain
                ...
                {
                    Items = actual,
                    Diff = actual.FirstDifferenceWith(expectedItems, equalityComparison),
                })
                .ForCondition(collectionWithDiff => collectionWithDiff.Diff.Index == -1)
        ...
        where T : INumber<T>
    {
        try
        {
            checked
            {
                ...
                return new FailReason("but {0} differs by {1} at index {2}.", collectionWithDiff.Items, difference, collectionWithDiff.Diff.Index);
            }
        }
        catch (OverflowException)
        {
            // If the difference is too large we (e.g. sbyte x = -127, sbyte y = 128), we cannot represent the difference.
            return new FailReason("but {0} differs at index {1}.", collectionWithDiff.Items, collectionWithDiff.Diff.Index);
        }
    }
    ...
    {
        assertionChain
            .WithExpectation("Did not expected collections to be approximately equal within ± {0}{reason}, ", precision, chain => chain
                .Given(() => subject)
